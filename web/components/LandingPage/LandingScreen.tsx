    ...
    >
      ...
      <div className="flex flex-col items-center justify-center -mt-4">
        <p className={`${styles.bottomText}`}>
          The National Police Index is a project and data tool showing police employment history data obtained from
          state police training and certification boards across the U.S. In total, 27 states have released centralized
          employment history data, 23 of which are currently represented on the data tool.
        </p>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 mt-2">
        {states.map((state) => (
          <button key={state} className={`${buttonStyles.stateButtons}`} onClick={() => handleStateClick(state)}>
            {state}
          </button>
        ))}
      </div>
      <LouisianaModal isOpen={isLouisianaModalOpen} onClose={() => setIsLouisianaModalOpen(false)} />
